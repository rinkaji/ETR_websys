...
<div class="row mb-4">
    ...
    <div class="col">
        <a href="{{ route('dashboard', ['low_stock' => 1]) }}#inventory" style="text-decoration: none;">
            <div class="card text-dark" style="background-color: #f1f1f1; border: none;">
                <div class="card-body">
...
@endif
<h2 id="inventory" class="h4 mt-5 mb-3">Supplies Inventory</h2>
    ...
    <table class="table table-bordered table-hover align-middle">
        ...
        <tbody>
            ...
            <tr>
                ...
                @php
                $supplySources = [
                164 => '164 - Supply Office',
                161 => '161 - IGP',
                184 => '184 - DOST',
                101 => '101- PSU Lingayen',
                ];
                @endphp
                <td>{{ $supplySources[$supply->supply_from] ?? 'NA' }}</td>
                <td>{{ number_format($supply->unit_cost, 2) }}</td>
                <td>{{ number_format($supply->supply_from_quantity * $supply->unit_cost, 2) }}</td>
                <td>{{ $supply->created_at->format('Y-m-d H:i:s') }}</td>
                <td>
                    ...
                    <a href="{{ route('admin.stockCard', [ 'item' => urlencode($supply->item), 'description' => urlencode($supply->description), 'unit' => urlencode($supply->unit)]) }}"
                        ...
                        <img class="icon-crud" src="{{ asset('images/print-icon.svg') }}"> View
                    </a>
